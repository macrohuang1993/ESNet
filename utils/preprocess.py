...
def default_transform(input_size=None, 
    ...
    if torchvision.__version__ == '0.2.0':
        rgb_list = [
            transforms.ToTensor(),
            transforms.Normalize(**normalize),
        ]
    elif torchvision.__version__ == '0.5.0+cu100':
            rgb_list = [
            transforms.ToTensor(),
            _div255(),
            transforms.Normalize(**normalize),
        ...
        raise Exception("Check ToTensor Behavior (wether /255 for float numpy array) of the torchvision used")
...
def unnormalize_imagenet_img(images):
    ...
    result_images[:, 0, :, :] = result_images[:, 0, :, :] * __imagenet_stats['std'][0] + __imagenet_stats['mean'][0]
    result_images[:, 1, :, :] = result_images[:, 1, :, :] * __imagenet_stats['std'][1] + __imagenet_stats['mean'][1]
    result_images[:, 2, :, :] = result_images[:, 2, :, :] * __imagenet_stats['std'][2] + __imagenet_stats['mean'][2]
    if is_three_dim:
...
def save_pfm(filename, image, scale = 1):
  #Zhengyu: May need to migrate this to python 3 as well, used in detect.sh 
...
def load_pfm(filename):
    #Next line is Python 3 version, from https://github.com/xingyul/flownet3d/blob/master/data_preprocessing/load_pfm.py
    file = open(filename, 'r', newline='', encoding='latin-1')  
    #file = open(filename, 'r') # Original version for python2
    color = None
    width = None
    height = None
    scale = None
