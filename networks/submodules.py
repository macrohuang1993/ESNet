...
class ResBlock(nn.Module):
    def __init__(self, n_in, n_out, stride = 1, deform = False):
        ...
        if deform:
            self.conv1 = DeformConv2d(n_in,n_out, kernel_size=3, stride = stride, dilation=2, deformable_groups=2,modulation=True)
            self.conv2 = DeformConv2d(n_out,n_out, kernel_size=3, stride = 1, dilation=2, deformable_groups=2,modulation=True)
        else:
...
def build_corr_two_side(img_left, img_right, search_range = range(-4,5)):
    """
    Building Correlation by search both left and right shift direction. 
    build_corr(img_left, img_right, max_disp=40) is same as build_corr_two_side(img_left, img_right, search_range = range(0,40)):
    """
    B, C, H, W = img_left.shape
    volume = img_left.new_zeros([B, len(search_range), H, W])
    for i, shift in enumerate(search_range):
        if shift == 0:
            volume[:, i, :, :] = (img_left[:, :, :, :] * img_right[:, :, :, :]).mean(dim=1)
        elif shift > 0:
            volume[:, i, :, shift:] = (img_left[:, :, :, shift:] * img_right[:, :, :, :-shift]).mean(dim=1)
        else:
            volume[:, i, :, :shift] = (img_left[:, :, :, :shift] * img_right[:, :, :, -shift:]).mean(dim=1)
    volume = volume.contiguous()
    return volume
...
if __name__ == "__main__":
    imgL, imgR = torch.rand(1,3,100,200),torch.rand(1,3,100,200)
    a = build_corr(imgL, imgR,max_disp=40)
    b= build_corr_two_side(imgL,imgR,  search_range=range(-40,40))
